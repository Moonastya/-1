...
public:
    DeusEx(string name = "Джин", int year = (rand() % 10000)) 
    {
        gd_name = name;
        gd_year = year;
    }
    ...
    string getGd_name() const 
    { 
        return gd_name; 
    }

    int getGd_year() const 
    { 
        return gd_year; 
    }
...
public:
    ...
    void output2() 
    { 
        cout << "С " << getGd_year() << " и по сей день"; 
    }
};
...
int main() 
{
    ...
    first.setName("Кот");
    first.setAge(5);
    first.setCatchPhrase("хочет узнать научится ли летать.");
    first.setMagmum(0, "Конечно.");
    first.setMagmum(1, "Возможно.");
    first.setMagmum(2, "Когда-нибудь.");
    first.setAGe(10);
    first.forecast();
    Believer F = Believer("Ангел", 10);
    F.output();
    first.bonus();
    F.output2();
    return 0;
